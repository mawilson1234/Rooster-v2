...
var language_arr = new Array("English", "Afrikaans", "Albanian", "Arabic", "Armenian", "Basque", "Bengali", "Bulgarian", "Catalan", "Cambodian", "Chinese (Mandarin)", "Chinese (Cantonese)", "Chinese (other)", "Croatian", "Czech", "Danish", "Dutch", "Estonian", "Fiji", "Finnish", "French", "Georgian", "German", "Greek", "Gujarati", "Hebrew", "Hindi", "Hungarian", "Icelandic", "Indonesian", "Irish", "Italian", "Japanese", "Javanese", "Korean", "Latin", "Latvian", "Lithuanian", "Macedonian", "Malay", "Malayalam", "Maltese", "Maori", "Marathi", "Mongolian", "Nepali", "Norwegian", "Persian", "Polish", "Portuguese", "Punjabi", "Quechua", "Romanian", "Russian", "Samoan", "Serbian", "Slovak", "Slovenian", "Spanish", "Swahili", "Swedish ", "Tamil", "Tatar", "Telugu", "Thai", "Tibetan", "Tonga", "Turkish", "Ukrainian", "Urdu", "Uzbek", "Vietnamese", "Welsh", "Xhosa", "Other (not listed)");
...
function populateCountries(countryElementId, stateElementId){
  ...
  countryElement.options[0].selected = true;
...
function populateAges( ageElementId ){
	var ageElement = document.getElementById( ageElementId );
	ageElement.length=0;
	ageElement.options[0] = new Option("Select Age", '-1');
	ageElement.selectedIndex = 0;
	ageElement.options[0].selected = true;
	ageElement.options[0].disabled = true;

	var ageArray = ['Do not wish to state'];
	for(var i = 18; i <= 99; i++){
		ageArray.push(i);
	}
	ageArray.push(">99");
	...
	for(var i = 0; i < ageArray.length; i++) {
		ageElement.options[ageElement.length] = new Option(ageArray[i], ageArray[i]);
	}
}
...
function populateLanguages ( languageElementId ){
	...
	languageElement.options[0] = new Option('Select Language', '-1');
	languageElement.selectedIndex = 0;
	languageElement.options[0].selected = true;
	languageElement.options[0].disabled = true;
